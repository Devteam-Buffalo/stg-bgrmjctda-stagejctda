...
<article id="post-<?php the_ID(); ?>" class="post-article">
	...
	<section class="uk-container uk-container-center single-location">
		<div class="uk-grid">
			<div class="uk-width-large-7-10 uk-width-1-1 location-details left-column">
				<div class="location-intro">
					<?= jc_page_intro( $post ) ?>

					<div class="uk-flex uk-flex-middle uk-padding-top location-share">
						<div class="uk-flex-item-none uk-text-uppercase text-orange">Share</div>
						<?php pods_view( 'resource/view/partial/cta/social-share.php', compact( array_keys( get_defined_vars() ) ) ) ?>
					</div>

					<?php jc_get_weather( get_post() ) ?>
				</div>
									...
									<div class="async-contents">
										<div class="uk-grid uk-grid-width-1-1 uk-dropdown-grid uk-grid-match" data-uk-grid-match="{target:'.uk-panel'}">
											<div class="uk-width-medium-1-3">
												<div class="uk-cover-background uk-position-relative uk-panel" style="background-image: url(<?= get_field('tile_image')['url'] ?>)">
													<img src="<?= get_field('tile_image')['url'] ?>" alt="Photo of <?= $post->post_title ?>" class="uk-invisible">
												</div>
											</div>
...
<!-- 						</div> -->
	...
	<div class="uk-block">
		<div class="uk-container uk-container-center">
			<h3 class="mountain"><?= get_post_type_object( get_post_type() )->label ?></h3>
			<?php jc_carousel_posts( [ 'post_type' => get_post_type() ] ) ?>
		</div>
	</div>
</article>