...
<style>.map-markers>.map-marker{display: none}</style>
						...
						<div>
							...
							<div class="uk-flex">
								<div class="uk-flex-item-none map-image">
									<?php
									$t = ( $child->post_type == 'outdoor' || $child->post_type == 'attraction' )
										? pods_image_url( get_post_meta( $child->ID, 'tile_image', true ) )
										: wp_get_attachment_image_url( get_post_meta( $child->ID, 'tile_image', true )
									); ?>
		...
		<div class="uk-block">
			...
			<?php foreach ( $children as $child ) : ?>
				<a href="<?= get_permalink( $child->ID ) ?>" title="<?php the_title_attribute( $child->ID ) ?>" class="masonry-tile masonry-link">
					<?php
					$t = ( $child->post_type == 'outdoor' || $child->post_type == 'attraction' )
						? pods_image_url( get_post_meta( $child->ID, 'masonry_image', true ) )
						: wp_get_attachment_image_url( get_post_meta( $child->ID, 'masonry_image', true )
					); ?>
