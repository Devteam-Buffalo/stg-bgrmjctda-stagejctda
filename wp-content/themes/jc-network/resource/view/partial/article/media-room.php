...
<div class="uk-container uk-container-center">
	...
	<div class="uk-grid uk-grid-large uk-block">
		...
		<article id="post-<?= get_the_id() ?>" class="uk-width-1-1 uk-width-large-2-3 post-article" role="article">
					...
					<div>
						...
						<article class="uk-panel uk-panel-header uk-panel-divider">
							<a href="<?= get_the_permalink() ?>" title="<?= the_title_attribute() ?>" class="uk-link-muted">
								...
								<footer>
									<time class="uk-badge uk-badge-primary" datetime="<?= date_i18n( 'c', strtotime( get_the_date() ) ) ?>"><?= get_the_date( get_option( 'date_format' ), $releases->ID ) ?></time>
								</footer>
							</a>
						</article>
					...
					<div>
						...
						<article class="uk-panel uk-panel-header uk-padding-remove uk-panel-divider">
							<a href="<?= get_the_permalink() ?>" title="<?= the_title_attribute() ?>" class="uk-link-muted">
								...
								<footer>
									<time class="uk-badge uk-badge-primary" datetime="<?= date_i18n( 'c', strtotime( get_the_date() ) ) ?>"><?= get_the_date( get_option( 'date_format' ), $b2b_news->ID ) ?></time>
								</footer>
							</a>
						</article>
						<?php endwhile; wp_reset_postdata(); ?>
						...
						<div>
							<article class="uk-panel uk-panel-header uk-padding-remove">
								<a href="<?= $attachment ?>" title="<?= the_title_attribute() ?>" class="uk-link-muted">
									...
									<footer>
										<time class="uk-badge uk-badge-primary" datetime="<?= date_i18n( 'c', strtotime( get_the_date() ) ) ?>"><?= get_the_date( get_option( 'date_format' ), $mentions->ID ) ?></time>
									</footer>
								</a>
							</article>
							<hr>
		...
		<aside id="aside-jc-media-room-sidebar" class="uk-width-1-1 uk-width-large-1-3 post-aside" role="complementary">
