...
if( ! class_exists('JCFrontEndLogin') ){

	class JCFrontEndLogin {
		 ...
		 */
		 public function init() {
			if ( is_admin() && ! current_user_can( 'administrator' ) && ! current_user_can( 'rawlemurdy' ) && ! current_user_can( 'jackson_county' ) &&
		 ...
		 public function after_setup_theme(){
			...
			if ( is_multisite() ) {
				...
				$login_header_url	= __( 'https://www.discoverjacksonnc.com/jc-login/' );
				$login_header_title = __( 'Discover Jackson County' );
			}
	...
	$JCFrontEndLogin = new JCFrontEndLogin();
...
//add_filter( 'wp_nav_menu_items', 'jc_loginout_menu_link', 10, 2 );

function jc_loginout_menu_link( $items, $args ) { ?>
<!-- 	<ul class="uk-subnav uk-subnav-line"> -->
		<?php if( ! current_user_can( 'edit_others_pages' ) ) : ?>
		<li>
			<a href="<?php echo get_permalink( get_page_by_title( 'help-support' ) ); ?>" class="uk-link-mute">
				<span class="uk-icon-question"></span> Help &amp; Support
			</a>
		</li>
		<?php endif; ?>
		...
		<li>
			<a href="<?php echo get_dashboard_url(); ?>" class="uk-link-mute">
				<span class="uk-icon-dashboard"></span> View Dashboard
			</a>
		</li>
		<?php endif; ?>

		<li>
			<a href="<?php echo get_permalink( get_page_by_title( 'edit-profile' ) ); ?>" class="uk-link-mute">
				<span class="uk-icon-pencil"></span> Edit Profile
			</a>
		</li>

		<li>
			<a href="<?php echo wp_logout_url( get_page_by_title( 'jc-login' ) ); ?>" class="uk-link-mute">
...
function redirect_after_user_registration() {
	return home_url( '/jc-login/' );
}
//add_filter( 'registration_redirect', 'redirect_after_user_registration' );
...
function redirect_after_user_logout( $redirect_to, $requested_redirect_to, $user ) {
	return home_url( '/jc-login/' );
}
//add_filter( 'logout_redirect', 'redirect_after_user_logout', 10, 3 );

function redirect_after_user_login( $redirect_to, $requested_redirect_to, $user ) {
	return home_url( '/jc-login/' );
}
//add_filter( 'login_redirect', 'redirect_after_user_login', 10, 3 );
...
/* ---------------------------------------
*
* Global User Login Redirect (Success)
*
------------------------------------ */
/*
add_filter( 'login_redirect', function( $redirect_to, $requested_redirect_to, $user ) {
	apply_filters( 'logout_redirect', $redirect_to, $requested_redirect_to, $user )
	$account_page	= get_permalink( get_page_by_title( 'jc-login' ) );
...
/* ---------------------------------------
*
* Global User Login Redirect (Empty)
*
------------------------------------ */
/*
...
/* ---------------------------------------
*
* Global User Login Redirect (Failed)
*
------------------------------------ */
/*
...
add_action( 'admin_notices', function() {
	...
	if ( isset( $user->roles ) && is_array( $user->roles ) ) {
		...
		if ( in_array( 'subscriber', $user->roles ) || in_array( 'location_owner', $user->roles ) || in_array( 'jackson_county', $user->roles ) ) {
...
add_action( 'admin_head', function() {
	...
	if ( isset( $user->roles ) && is_array( $user->roles ) ) {
		...
		if ( in_array( 'subscriber', $user->roles ) || in_array( 'location_owner', $user->roles ) || in_array( 'jackson_county', $user->roles ) ) {
			...
			echo '<style>
				.wpallexport-plugin .columns-to-export-content ol#columns { min-height: 235px; height: 600px; resize: vertical; }
				#wpbody-content * { font-size: inherit !important; }
			</style>';
		}
...
add_action( 'login_enqueue_scripts', function() {
	$jc_logo = get_stylesheet_directory_uri() . '/assets/img/logo-header-vector.svg';
